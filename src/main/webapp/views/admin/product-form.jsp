    ...
    <div class="panel form-shell">
        ...
        <form action="${formAction}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-select${not empty errors.categoryId ? ' is-invalid' : ''}" name="categoryId">
                    <c:forEach items="${categories}" var="category">
                        <option value="${category.categoryid}" ${not empty formData.categoryId ? (formData.categoryId == category.categoryid ? 'selected' : '') : (product.category != null and product.category.categoryid == category.categoryid ? 'selected' : '')}><c:out value="${category.categoryname}"/></option>
                    </c:forEach>
                </select>
            ...
            <c:if test="${not empty product.image}">
                <c:choose>
                    <c:when test="${fn:startsWith(product.image, 'http://') or fn:startsWith(product.image, 'https://')}"><img class="preview-image" src="${fn:escapeXml(product.image)}" alt="${fn:escapeXml(product.productName)}"></c:when>
                    <c:otherwise><img class="preview-image" src="<c:url value='/image?fname=${product.image}'/>" alt="${fn:escapeXml(product.productName)}"></c:otherwise>
                </c:choose>
            </c:if>
            <div class="form-group">
            ...
            <div class="form-actions">
                ...
                <a class="btn btn-secondary" href="<c:url value='/admin/users'/>">Users</a>
            </div>
        </form>
    </div>
</div>
